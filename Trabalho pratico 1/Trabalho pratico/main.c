...
int verificarJogadas(char matriz[][TAMANHO], char token, int linha, char coluna) {
    ...
    for (i = 0; i < TAMANHO; ++i) {
        if (coluna == (64 + i) && linha > 0 && linha < TAMANHO && matriz[linha][i] == ESPACO) {
            matriz[linha][i] = token;
            ++i;
        } else if (coluna == (64 + i) && linha > 0 && linha < TAMANHO && matriz[linha][i] != ESPACO) {
            puts("Jogada impossivel de ser realizada, a posicao esta ocupada por outro token! ");
            return 0;
            break;
        } else if (linha < 0 || linha > TAMANHO || coluna > 'I') {
            puts("Jogada impossivel de ser realizada, a posicao nao existe! ");
            return 0;
        } else if ((matriz[linha][coluna] == token) && (linha != 0 && coluna != 'Z')) { // deixar o jogador desistir
            return 0;
            break;
        }
    }
...
int Jogadas(char matriz[][TAMANHO], char token[JOGADORES]) {
    ...
    do {
        ...
        if (linha1 == 0 && coluna1 == 'Z') {
            ...
            break;
        } else if (numero_jogadas > 49) {
            printf("O jogo termina empatado!\n");
            break;
        }
        ++numero_jogadas;
    } while (numero_jogadas < 50); // maximo jogadas
}
